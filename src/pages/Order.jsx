        ...
        <section className='flex gap-5 justify-center flex-wrap lg:flex-nowrap px-5'>
          <article className='bg-white w-full lg:w-2xl mb-20 rounded-md'>
              ...
              <div className='flex flex-col sm:flex-row m-5 gap-5'>
                {movie && movie.poster_path ? (
                  <img 
                    ...
                    alt={movie.title}
                    className='w-full sm:w-60 h-auto sm:h-35 rounded object-cover' 
                  />
                ) : (
                ...
                <div className='w-full sm:w-100'>
                  ...
                  <div className='flex gap-3 flex-wrap mt-2'>
                    {movie?.genres?.slice(0, 2).map(genre => (
                      <p key={genre.id} className='bg-[#A0A3BD1A] p-1 pr-3 pl-3 rounded-full text-[#A0A3BD] text-sm sm:text-base'>
                        {genre.name}
                      </p>
                    ))}
                  </div>
                  <div className='flex flex-col sm:flex-row justify-between mt-3 gap-2'>
                    <p className='font-semibold text-sm sm:text-base'>Regular - 13:00 PM</p>
                    <button className='bg-[#1D4ED8] text-white p-1 pr-3 pl-3 rounded-md hover:bg-[#1a45b8] transition text-sm sm:text-base'>
                      Change
                    </button>
                  </div>
                </div>
            ...
            <div className='m-5 mt-8 mb-20'>
              ...
              <p className='text-[#4E4B66] text-center mt-8'>Screen</p>
                ...
                <div className='flex justify-center gap-4 sm:gap-10 mt-8 min-w-max px-4'>
                  <div className='grid grid-cols-8 gap-2'>
                    {rows.map(row => (
                      <React.Fragment key={row}>
                        <span className='flex items-center justify-center text-sm'>{row}</span>
                        {leftSeats.map(num => {
                          const seat = `${row}${num}`;
                          ...
                          return (
                            <span
                              key={seat}
                              onClick={() => status !== 'sold' && handleSeatClick(seat)}
                              className={`w-6 h-6 rounded-md ${getSeatColor(status)}`}
                            ></span>
                          );
                        })}
                      </React.Fragment>
                    ...
                    <span></span>
                    {leftSeats.map(num => (
                      <span key={num} className='text-center text-xs'>{num}</span>
                    ))}
                  </div>
                  ...
                  <div className='flex flex-col gap-2'>
                    {rows.map(row => (
                      <div key={row} className='flex gap-2'>
                        {rightSeats.map(num => {
                          const seat = `${row}${num}`;
                          const status = getSeatStatus(seat);
                          const isF10 = seat === 'F10';
                          const isF11 = seat === 'F11';
                          
                          if (isF11) return null; 
                          
                          return (
                            <span
                              key={seat}
                              onClick={() => status !== 'sold' && handleSeatClick(seat)}
                              className={`h-6 rounded-md ${getSeatColor(status)} ${
                                isF10 ? 'w-14' : 'w-6'
                              }`}
                            ></span>
                          );
                    ...
                    <div className='flex gap-2'>
                      {rightSeats.map(num => (
                        <span key={num} className='text-center text-xs w-6'>{num}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div className='mt-8'>
                <p className='text-lg sm:text-xl font-semibold'>Seating Key</p>
                <div className='flex flex-wrap justify-center gap-4 sm:gap-10 mt-5 text-sm sm:text-base'>
                  <p className='text-[#4E4B66]'>Available</p>
                  <div className='flex gap-2 items-center'>
                    <p className='bg-[#1D4ED8] w-6 h-6 rounded-md'></p>
                    <p className='text-[#4E4B66]'>Selected</p>
                  </div>
                  <div className='flex gap-2 items-center'>
                    <p className='bg-[#F589D7] w-6 h-6 rounded-md'></p>
                    <p className='text-[#4E4B66]'>Love Nest</p>
                  </div>
                  <div className='flex gap-2 items-center'>
                    <p className='bg-[#6E7191] w-6 h-6 rounded-md'></p>
            ...
            <article className='bg-white h-fit rounded-md w-full lg:w-auto'>
              <div className='m-6 sm:m-10 flex flex-col'>
                <div className='flex justify-center'>
                  <img src={CineOneLogo} alt="CineOne Logo" className="w-32 sm:w-36 lg:w-40 xl:w-45 h-auto" />
                </div>
                ...
                <div className='flex flex-col gap-5 text-sm sm:text-base'>
                  <div className='flex justify-between gap-5'>
                    <p className='text-[#6B6B6B] font-semibold'>Movie selected</p>
                    <p className='font-semibold text-right'>{movie?.title || 'N/A'}</p>
                  </div>
                  <div className='flex justify-between gap-5'>
                    <p className='text-[#6B6B6B] font-semibold'>Tuesday, 07 july 2020</p>
                    <p className='font-semibold'>13:00</p>
                  </div>
                  <div className='flex justify-between gap-5'>
                    <p className='text-[#6B6B6B] font-semibold'>One ticket price</p>
                    <p className='font-semibold'>${ticketPrice}</p>
                  </div>
                  <div className='flex justify-between gap-5'>
                    <p className='text-[#6B6B6B] font-semibold'>Seat Choosed</p>
                    <p className='font-semibold text-right'>
                      {displaySeats.length > 0 ? displaySeats.join(', ') : 'No seats selected'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className='flex justify-between pl-10 pr-10 pb-5'>
                <p className='text-xl font-bold'>Total Payment</p>
                <p className='text-2xl font-bold text-[#1D4ED8]'>$30</p>
              </div>
            </article>
            <article className='mt-10 mb-10 w-full lg:w-auto'>
              <button 
                className='bg-[#1D4ED8] p-4 sm:p-5 px-16 sm:px-35 rounded-lg text-white text-lg sm:text-xl hover:bg-[#1a45b8] transition w-full lg:w-auto'
                disabled={displaySeats.length === 0}
